...
def query_endpoints() -> dict:

    """
    Fetches data from kafka queue and sql database.
    
    Returns:
        dict: A dictionary containing the responses from various endpoints.
    """
    ...
    for key, value in endpoints.items():
        try:
            ...
            if response.status_code == 200:
                ...
                logger.debug(f"Failed to fetch data from {value}. Status Code: {response.status_code}, Response: {response.text}")
...
def run_consistency_checks() -> dict:
    ...
    """
    Perform consistency checks by comparing trace IDs across 
    kafka queue and database, write result to json.

    Returns:
        dict: A successful message with status code
    """
    ...
    not_in_queue = [item for item in eventstore_data['lst_in_db'] if item["trace_id"] not in trace_ids_in_anal]
    end_time = time.time()
    ...
    results = {
        ...
        "counts": {
            "db": {
                "online_orders": eventstore_data['count_in_db']['online_order_count'],
                "store_sales": eventstore_data['count_in_db']['store_sale_count']
            },
            "queue": {
                "online_orders": eventstore_data['counts_in_anal']['num_online_orders'],
                "store_sales": eventstore_data['counts_in_anal']['num_store_sales']
            },
            "processing": {
                "online_orders": eventstore_data['counts_in_proc']["num_online_orders"],
                "store_sales": eventstore_data['counts_in_proc']["num_store_sales"]
            }
        },
...
def get_checks():
    """
    Retrieves the check results from the json file

    Returns:
        dict: json data from the file if successful, otherwise an error 
